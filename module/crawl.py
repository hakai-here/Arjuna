...
from data.data import DRIVE_LINKS, EXTERNAL_LINKS, HANDLES_LINKS, HEADERS, IMG_SRC, MAIL_TO, NUMBERS, PDF, TINY_URLS, VISITED_LINKS, YOUTUBE_LINKS
import requests
import threading
...
def record(x,y): #please optimise using for loop and format
    print(f"\n Your Run Has Been recored in as {y} . Before any run it non related to {x} please flush data stored by typing 'flush' ")
    with open(x+"_crawl.txt",'wt') as f:
        if VISITED_LINKS:
            f.write("\n\n[*]Links :\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n")
            for i in VISITED_LINKS:
                f.write(i+"\n")
